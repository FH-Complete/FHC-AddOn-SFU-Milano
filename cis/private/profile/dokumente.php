...
echo "<br><hr>";
echo $p->t('global/studienjahr')."</b> <SELECT name='stsem' onChange=\"MM_jumpMenu('self',this,0)\">";
	foreach ($stsem_obj->studiensemester as $semrow)
	{
		// Studienjahr
        if (substr($semrow->studiensemester_kurzbz, 0, 2) == 'WS')
        {
            $jahr = substr($semrow->studiensemester_kurzbz, 2, 4);
            $studienjahr = $jahr . "/" . substr(($jahr + 1), 2, 2);
        }
        else
            continue;

        if($jahr == $stjahr)
			echo "<OPTION value='dokumente.php?stjahr=$jahr' selected>$studienjahr</OPTION>";
		else
			echo "<OPTION value='dokumente.php?stjahr=$jahr'>$studienjahr</OPTION>";
	}
	echo "</SELECT><br />";
...
{
	echo "<a href='../../../../../cis/private/pdfExport.php?xsl=DichiaSost&xml=student.rdf.php&ss=".$stsem."&uid=".$uid."&xsl_stg_kz=".$xsl_stg_kz."'>".$p->t('tools/inskriptionsbestaetigung')."</a>";
