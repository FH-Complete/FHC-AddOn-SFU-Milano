...
if($person->foto!='')
{
	if(!($ansicht && $user->foto_sperre))
		echo '<img id="personimage" src="../../../../../cis/public/bild.php?src=person&person_id='.$user->person_id.'" alt="'.$user->person_id.'" height="100px" width="75px">';
	else
...
if(!$ansicht)
{
	...
	if(!$fs->akzeptiert($user->person_id))
		echo "<br><a href='#BildUpload' onclick='window.open(\"../../../../../cis/private/bildupload.php?person_id=$user->person_id\",\"BildUpload\", \"height=800,width=800,left=0,top=0,hotkeys=0,resizable=yes,status=no,scrollbars=yes,toolbar=no,location=no,menubar=no,dependent=yes\"); return false;'>".$p->t('profil/bildHochladen')."</a>";
}
if($user->foto_sperre)
...
if ($type == "student")
	echo $p->t('global/matrikelnummer').': '.$person->matr_nr.'  <br><br>';
...
echo $p->t('global/nachname').': '.$user->nachname.'<br><br>';
...
// interne Emailadresse ausgeblendet
