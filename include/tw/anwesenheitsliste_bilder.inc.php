...
$pdf->MultiCell(0,20,'Elenco studenti con foto',0,'L',0);
$pdf->SetFont('Arial','',10);
$pdf->SetXY(30,60);
$pdf->MultiCell(0,20,'Corso: '.$bezeichnung);
...
$pdf->MultiCell(0,20,'Semestre: '.(($semester->beschreibung != NULL) ? $semester->beschreibung : $stsem));
...
$maxY=$pdf->GetY();
$pdf->tablewidths = array(20,140,120,60,60); //Spaltenbreiten setzen
...
$pdf->SetFont('Arial','',8);
		...
		$pdf->MultiCell(130,$lineheight,mb_convert_encoding('Studente','ISO-8859-15','UTF-8'),1,'L',0);
		$maxX +=130;
		$pdf->SetXY($maxX,$maxY);
		$pdf->MultiCell(125,$lineheight,'Matricola',1,'C',0);
		$maxX +=125;
		$pdf->SetXY($maxX,$maxY);
		$pdf->MultiCell(65,$lineheight,'',1,'C',0);
		$maxX +=65;
		//$pdf->SetXY($maxX,$maxY);
...
if($result = $db->db_query($qry))
{
	...
	while($elem = $db->db_fetch_object($result))
	{
		...
		if($elem->status!='Abbrecher' && $elem->status!='Unterbrecher')
		{
			...
			$matrikelnr=trim(mb_convert_encoding($elem->matr_nr,'ISO-8859-15','UTF-8'));
			...
			$pdf->SetXY($maxX,$maxY);
			$pdf->SetFont('Arial','',10);
			$pdf->MultiCell(125,$lineheight,trim($matrikelnr),1,'C',1);
			$maxX +=125;
//			$pdf->SetXY($maxX,$maxY);
